defmodule ExSolomonWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ExSolomonWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert AccountsQueries.get_user_by_session_token(token)
    end
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/"
      refute AccountsQueries.get_user_by_session_token(user_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "users_sessions:abcdef-token"
      ExSolomonWeb.Endpoint.subscribe(live_socket_id)
